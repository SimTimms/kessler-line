...
function IconMouseMove({ active }: { active: boolean }) {
  return (
    ...
    >
      <svg
        viewBox="0 0 56 56"
        className="tutorial-key-hint__svg tutorial-key-hint__svg--move"
        aria-hidden
      >
    ...
    >
      <svg viewBox="0 0 40 56" className="tutorial-key-hint__svg" aria-hidden>
        ...
        <rect
          ...
          height="14"
          rx="2"
          className="tutorial-key-hint__scroll-bump"
        />
      ...
      {hints.map((hint, i) => {
        ...
        switch (hint.kind) {
          case 'keyboard': {
            ...
            return <KeyCap key={key} label={label} active={pressedKeyLabels.has(label)} />;
          }
          case 'mouseLeft':
