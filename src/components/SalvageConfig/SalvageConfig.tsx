...
function applySalvageConfigScannerDefaults(): void {
  ...
  magneticScanRangeRef.current = getScannerRange(
    'magnet',
    SALVAGE_CONFIG_SCANNER_INITIAL_POWERS.magnet
  );
  driveSignatureOnRef.current = true;
  driveSignatureRangeRef.current = getScannerRange(
    'drive',
    SALVAGE_CONFIG_SCANNER_INITIAL_POWERS.drive
  );
  proximityScanOnRef.current = true;
  proximityScanRangeRef.current = getScannerRange(
    'proximity',
    SALVAGE_CONFIG_SCANNER_INITIAL_POWERS.proximity
  );
  radioOnRef.current = true;
  radioRangeRef.current = getScannerRange('radio', SALVAGE_CONFIG_SCANNER_INITIAL_POWERS.radio);
}

export default function SalvageConfig() {
  ...
  useEffect(() => {
    ...
    resetScannerRefs();
    applySalvageConfigScannerDefaults();
  }, []);
