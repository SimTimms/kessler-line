...
import {
  EVENT_TORPEDO_HIT,
  TORPEDO_EXPLOSION_COLOR,
  TORPEDO_EXPLOSION_DRAG,
  TORPEDO_EXPLOSION_LIFETIME,
  TORPEDO_EXPLOSION_MAX_PARTICLES,
  TORPEDO_EXPLOSION_PARTICLES_PER_BURST,
  TORPEDO_EXPLOSION_SIZE,
  TORPEDO_EXPLOSION_SPEED_MAX,
  TORPEDO_EXPLOSION_SPEED_MIN,
  TORPEDO_EXPLOSION_SPREAD,
...
/**
 * Particle-burst explosion VFX for torpedo detonations.
 * Listens for {@link EVENT_TORPEDO_HIT}; ring-buffer pool pattern.
 */
export default function TorpedoExplosion() {
  const anchorRef = useRef<THREE.Group>(null!);
  const geoRef = useRef<THREE.BufferGeometry>(null!);
  const pool = useRef(makePool(TORPEDO_EXPLOSION_MAX_PARTICLES));
  const slot = useRef(0);

  const positions = useMemo(
    () => new Float32Array(TORPEDO_EXPLOSION_MAX_PARTICLES * 3),
    []
  );
  ...
  useEffect(() => {
    const onHit = (event: Event) => {
      ...
      for (let i = 0; i < TORPEDO_EXPLOSION_PARTICLES_PER_BURST; i++) {
        const idx = slot.current;
        slot.current = (idx + 1) % TORPEDO_EXPLOSION_MAX_PARTICLES;
        const s = pool.current[idx]!;

        randomConeDirection(_normal, TORPEDO_EXPLOSION_SPREAD, _dir);
        const speed =
          TORPEDO_EXPLOSION_SPEED_MIN +
          Math.random() *
            (TORPEDO_EXPLOSION_SPEED_MAX - TORPEDO_EXPLOSION_SPEED_MIN);

        s.active = true;
        s.age = 0;
        s.maxAge = TORPEDO_EXPLOSION_LIFETIME * (0.6 + Math.random() * 0.6);
        s.px = detail.point.x + _normal.x * 0.5;
        s.py = detail.point.y + _normal.y * 0.5;
        s.pz = detail.point.z + _normal.z * 0.5;
        s.vx = _dir.x * speed;
        s.vy = _dir.y * speed;
        s.vz = _dir.z * speed;
      }
  ...
  useFrame((_, delta) => {
    ...
    _color.set(TORPEDO_EXPLOSION_COLOR);
    const drag = Math.max(0, 1 - TORPEDO_EXPLOSION_DRAG * delta);

    for (let i = 0; i < TORPEDO_EXPLOSION_MAX_PARTICLES; i++) {
      ...
      const fade = lifeT < 0.2 ? 1 : 1 - (lifeT - 0.2) / 0.8;
