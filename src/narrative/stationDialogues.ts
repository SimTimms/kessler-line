// Branching dialogue trees for station-resident NPCs. Self-contained — these do
// NOT reuse the radio-hail PlayerOption type, so ship-hail assignment is
// unaffected. Player options carry optional `effects` (see dialogueEffects.ts)
// that mutate game state when chosen.

import type { DialogueEffect } from './dialogueEffects';

export interface StationPlayerOption {
  id: string;
  /** Short button label. */
  label: string;
  /** Full player line shown in the thread. */
  text: string;
  /** Next turn, or null to end the conversation. */
  nextTurnId: string | null;
  /** State changes applied when this option is chosen. */
  effects?: DialogueEffect[];
}

export interface StationDialogueTurn {
  id: string;
  npcText: string;
  playerOptions: StationPlayerOption[];
}

export interface StationDialogueTree {
  id: string;
  openingTurnId: string;
  turns: Record<string, StationDialogueTurn>;
}

// ── Dockmaster: Vance Korr, Helix Port Authority ───────────────────────────────
// Exercises every effect type: paid refuel (transferResource), a courier parcel
// (giveCargo + shareInfo), and a shady off-manifest branch with chance-based
// theft / hull damage.

const DOCKMASTER_TREE: StationDialogueTree = {
  id: 'dockmaster',
  openingTurnId: 'intro',
  turns: {
    intro: {
    ...
    'work-accepted': {
      id: 'work-accepted',
      npcText: "Good. It's in your hold and the coordinates are on your inbox. Fly safe — that crate's worth more than your ship.",
      playerOptions: [
        {
          id: 'work-ack',
          label: 'Understood',
...
export const STATION_DIALOGUE_TREES: Record<string, StationDialogueTree> = {
  [DOCKMASTER_TREE.id]: DOCKMASTER_TREE,
};

export function getStationDialogueTree(id: string): StationDialogueTree | undefined {
  return STATION_DIALOGUE_TREES[id];
}
